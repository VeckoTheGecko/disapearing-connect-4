...
icon=pygame.image.load('four.png')
pygame.display.set_icon(icon)

#loading the player
connect4Image=pygame.image.load('connect4.png')#800*700
greytokenImage=pygame.image.load('greytoken.png')#88*88
redtokenImage=pygame.image.load('redtoken.png')
yellowtokenImage=pygame.image.load('yellowtoken.png')
...
def iswin(board,turn,row,pos): 
    ...
    for condition in winconditions:
        ...
        while board[tempcoord[0]][tempcoord[1]]==turn:
            if 0<=tempcoord[0]-condition[0]<=5 and 0<=tempcoord[1]-condition[1]<=6:
                if board[tempcoord[0]-condition[0]][tempcoord[1]-condition[1]]==turn:
                ...
                if state=='ready'and game!='over':
                    ...
                    if row!='invalid move':
                        ...
                        if iswin(board,turn,row,pos):
                            ...
                            turn='red'
                    if game!='over' and '' not in board[5]:
                        game='over'
                        turn='nobody'
                    else:
                        state='ready'
    ...
    if game!='over':
        for i in range(len(board)):
            for j in range(len(board[i])):
                if board[i][j]=='red' and (i,j) in last2moves:
                    red(columnsX[j],rowsY[i])
                elif board[i][j]=='yellow' and (i,j) in last2moves:
                    yellow(columnsX[j],rowsY[i])
                elif (board[i][j]=='red' or board[i][j]=='yellow') and (i,j) not in last2moves:
                    grey(columnsX[j],rowsY[i])
                else:
                    pass
    if game=='over':
        for i in range(len(board)):
